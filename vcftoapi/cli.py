...
@click.group()
@click.version_option(prog_name='vcftoapi', version=0.01)
def main():
    """This is the vcftoapi CLI"""
    pass
...
@click.command()
@click.option('--host', default='0.0.0.0', help='IP address to bind the vcftoapi server to', show_default=True)
@click.option('--port', default='8080', help='Port number to bind the vcftoapi server to', show_default=True)
@click.option('--infer-config', is_flag=True, help='If set: infer a basic configuration from the provided VCF and GFF/GFF3 files and do not use an existing `vcftoapi.config.yml`')
...
def start(host: str, port: str, infer_config: bool, save_config): 
    ...
    if infer_config:
        ...
        click.secho('The following VCF files have been found. Please choose the one you want to visualize with vcftoapi:', fg='yellow')
        i = 0
        for _vcf_file in vcf_files:
            click.secho('['+str(i)+'] '+os.path.basename(_vcf_file), fg='yellow')
            i = i + 1
        ...
        click.secho('The following GFF/GFF3 files have been found. Please choose the one you want to visualize with vcftoapi:', fg='yellow')
        i = 0
    ...
    click.secho('Starting vcftoapi server...', fg='green', bold=True)
    ...
    click.secho('vcftoapi should be available under the following URL: '+str(url), fg='green', bold=True)
